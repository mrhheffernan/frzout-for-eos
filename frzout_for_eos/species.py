...
def _read_particle_data():
    ...
    #this file lists antiparticles for mesons but not baryons
    ...
    with open('pdg-urqmd_v3.3+.dat', 'r') as pdgfile:
        ...
        for line in lines:
            ...
            #add to lists for testing
            test_ID.append(ID)
            test_name.append(name)
            test_mass.append(mass)
            test_degen.append(degen)
            with open('test_ID_mass.txt', 'w') as f:
                f.write("ID" + "\t" "mass\n")
                for i in range( len( test_ID ) ):
                    f.write( str(test_ID[i])+ "\t" + str(test_mass[i]) + "\n")

            #pdg file lists antimesons but not antibaryons
            has_anti = (baryon > 0)
            base_data = dict(
                name=name,
                mass=mass,
